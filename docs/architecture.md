...
| **Journal** | `add_journal_note` — observations and decisions about how the developer works with agents (optionally scoped by `project_id`) | Human in `Insights → Journal` UX (reviews, rewrites, deletes; daily digest engine from v2.18.0) | `recall_journal` — at most 3 curated entries within a 1,000-token cap |
...
- `/insights/journal` (Journal) — timeline of agent-usage observations, decisions about agent use, and agent-filed notes. Minimal timeline in v2.17.0; digests and standup in v2.18.0.
