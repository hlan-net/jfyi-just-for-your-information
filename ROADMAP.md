...
| MCP SDK 2.x migration (`mcp>=2`): port `build_mcp_server` to the constructor `on_*` handlers and replace `Server.run(stateless=True)` in the Streamable HTTP handler. Prerequisite: tests for `_build_streamable_handler` on 1.x first | Planned | — | Infrastructure |

- **Journal UI (`/insights/journal`):** Timeline, daily digest cards, quick-entry bar, standup export. Journal content is agent-usage tracking, not per-project task tracking ([docs/journal.md](docs/journal.md)).
...
- **MCP SDK 2.x migration:** `mcp` 2.x (protocol revision 2026-07-28) removes the low-level `Server` decorators (`@server.list_tools()` etc., replaced by `Server(on_list_tools=…, on_call_tool=…, …)`) and the `stateless=` argument of `Server.run`, which the Streamable HTTP handler (`cli.py`) relies on. v2.17.2 keeps `mcp>=1.28.1,<2`. Tests for the Streamable handler land first, on 1.x, so the port is verified against the plugin's only transport.
